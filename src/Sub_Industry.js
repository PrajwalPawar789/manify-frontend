...
const IndustryFilter = ({ selectedSubIndustries, handleSubIndustrySelection, SubindustrySearchTerm, handleSubIndustrySearchChange }) => {
  const industryFilter = filters.find((f) => f.id === "sub_industry");
  const filteredIndustryOptions = industryFilter.options.filter((option) =>
    option.label.toLowerCase().includes(SubindustrySearchTerm)
  );
          ...
          <Disclosure.Panel className="pt-4 ">
            <div className="pb-2">
              <input
                ...
                onChange={handleSubIndustrySearchChange}
                value={SubindustrySearchTerm}
              />
            </div>
                ...
                {filteredIndustryOptions.map((option, optionIdx) => (
                  <div key={option.value} className="flex items-center">
                    <input
                      ...
                      value={option.value}
                      type="checkbox"
                      checked={selectedSubIndustries.includes(option.value)}
                      onChange={() => handleSubIndustrySelection(option.value)}
